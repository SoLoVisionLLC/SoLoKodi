...
| --- | --- | --- | --- | --- |
| sololink.cloud | access-protected | meets_standard | none | wildcard retained by policy: Coolify preview/app namespace; wildcard covers many intentionally parked or powered-down preview hosts.; Cloudflare SSL mode could not be read with the current token; Always Use HTTPS setting could not be read with the current token; Automatic HTTPS Rewrites setting could not be read with the current token |
...
| --- | --- | --- | --- | --- | --- | --- |
| solokodi.sololink.cloud | / | coolify-root | advisory | 503 | 503 | no available server from Cloudflare and direct origin |
| solokodi.sololink.cloud | /api/health | health-candidate | advisory | 503 | 503 | no available server from Cloudflare and direct origin |
| solokodi.sololink.cloud | /health | health-candidate | advisory | 503 | 503 | no available server from Cloudflare and direct origin |
| solokodi.sololink.cloud | /healthz | health-candidate | advisory | 503 | 503 | no available server from Cloudflare and direct origin |

Current route findings:

- Failing required routes in this repo snapshot: none detected
- Advisory routes in this repo snapshot: 4
